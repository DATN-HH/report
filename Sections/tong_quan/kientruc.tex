...
Kiến trúc hệ thống đóng vai trò nền tảng trong dự án hệ thống quản lý nhà hàng. Việc áp dụng \textbf{Layered Architecture} giúp phân tách chức năng thành nhiều tầng độc lập, đảm bảo tính rõ ràng, dễ bảo trì và dễ mở rộng. Mỗi tầng có trách nhiệm riêng biệt nhưng được kết nối chặt chẽ, từ đó tối ưu hoá việc quản lý, nâng cấp, tích hợp và mở rộng trong tương lai.

\subsubsection*{Các tầng trong kiến trúc hệ thống}
\begin{itemize}
    \item \textbf{User Interface Layer}: 
    Hiển thị giao diện và xử lý tương tác của người dùng, được phát triển bằng ReactJS, TanStack và ShadCN.

    \item \textbf{Presentation Layer}: 
    Tiếp nhận và xử lý các yêu cầu HTTP từ frontend, định tuyến đến các API thông qua Spring Boot RestController.

    \item \textbf{Application Layer}: 
    Điều phối logic nghiệp vụ, gọi tới các Service để thực thi các quy tắc kinh doanh.

    \item \textbf{Domain Layer}: 
    Định nghĩa các mô hình nghiệp vụ (Entity) và ánh xạ với dữ liệu thực tế, đảm bảo sự nhất quán giữa logic và dữ liệu.

    \item \textbf{Persistence Layer}: 
    Quản lý truy cập dữ liệu bằng Repository, triển khai cơ chế ORM thông qua Hibernate để giao tiếp với cơ sở dữ liệu.

    \item \textbf{Data Layer}: 
    Lưu trữ dữ liệu lâu dài trong PostgreSQL, bao gồm thông tin về thực đơn, đơn hàng, khách hàng, nhân viên và các nghiệp vụ liên quan.

    \item \textbf{Infrastructure Layer}: 
    Cung cấp các dịch vụ hạ tầng hỗ trợ, bao gồm cache với Redis, ORM (Hibernate), các module Spring Boot (Security, Logging, Config, Messaging...), đồng thời kết nối với các ứng dụng/dịch vụ bên ngoài.
\end{itemize}
