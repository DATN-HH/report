...
\begin{figure}[H]
	\centering
	\includegraphics[width=12cm]{Images/site_map/Site_map}
	\vspace{0.5cm}
	\caption{Sitemap của cho các trang chung}
	\label{fig:my_label}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Images/site_map/staff1}
    \vspace{0.5cm}
    \caption{Sitemap của cho các trang quản lý (1)}
    \label{fig:my_label}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Images/site_map/Staff2}
    \vspace{0.5cm}
    \caption{Sitemap của cho các trang quản lý (2)}
    \label{fig:my_label}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=13cm]{Images/site_map/SiteRole1}
    \vspace{0.5cm}
    \caption{Sitemap theo vai trò người dùng (Nhân viên và Khách hàng)}
    \label{fig:my_label}
\end{figure}
%\begin{landscape}
%
%    % The 'figure' environment holds our images and caption.
%    % The [!htbp] specifier is a bit more flexible than [p].
%    \begin{figure}[!htbp]
%        \centering % Center the entire content horizontally on the page
%
%        % --- LEFT IMAGE ---
%        % The [b] option aligns the minipages at their bottom.
%        \begin{minipage}[b]{0.49\textwidth}
%            \centering % Center the image within this column
%            \includegraphics[
%                width=\linewidth,      % Set image width to 100% of the minipage width
%                height=0.9\textheight, % Limit height to 80% of page height
%                keepaspectratio        % IMPORTANT: Prevents image distortion
%            ]{Images/site_map/staff1}
%        \end{minipage}% <-- The % here is crucial. It removes any unwanted space/newline.
%        \hfill % This command creates a flexible space that pushes the two images apart
%        % --- RIGHT IMAGE ---
%        \begin{minipage}[b]{0.49\textwidth}
%            \centering % Center the image within this column
%            \includegraphics[
...
%                height=0.9\textheight, % Limit height to 80% of page height
%                keepaspectratio        % IMPORTANT: Prevents image distortion
%            ]{Images/site_map/Staff2}
%        \end{minipage}
%
%        \vspace{1cm} % Add a little vertical space before the caption
%
%        % --- CAPTION for BOTH images ---
%        \caption{Sitemap cho hệ thống quản lý nhà hàng}
%        \label{fig:sitemap_staff_side_by_side} % A unique label for referencing
%
%    \end{figure}
%
%\end{landscape}
...
%\begin{figure}[H]
%    \centering
%    \includegraphics[width=20cm]{Images/site_map/SiteRole1}
%    \vspace{0.5cm}
%    \caption{Sitemap cho khách hàng, nhân viên}
%    \label{fig:my_label}
%\end{figure}
%\begin{landscape}

    % Use the figure environment as usual.
    % The [p] specifier is good for full-page figures.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Images/site_map/ManagerSite}
    \vspace{0.5cm}
    \caption{Sitemap theo vai trò người dùng (Quản lý) (1)}
    \label{fig:my_label}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=13cm]{Images/site_map/ManagerSite2}
    \vspace{0.5cm}
    \caption{Sitemap theo vai trò người dùng (Quản lý) (2)}
    \label{fig:my_label}
\end{figure}

\begin{longtable}{
    >{\raggedright\arraybackslash}p{2.5cm} % Cột 1: Vai trò
    >{\raggedright\arraybackslash}p{4.5cm} % Cột 2: Tính năng
    p{8cm}  % Cột 3: Mô tả
}
\caption{Sitemap Chi Tiết của Hệ thống Quản lý Nhà hàng} \\
\toprule
\textbf{Vai trò} & \textbf{Tính năng / Mô-đun} & \textbf{Mô tả} \\
\midrule
\endfirsthead % Header cho trang đầu tiên
...
\textbf{Khách (Guest)}
    & Trang chủ (Homepage)
    & Giao diện chính, giới thiệu nhà hàng, các chức năng chính. \\
\cmidrule(lr){2-3}
    & Giới thiệu (About Us)
    & Cung cấp thông tin chi tiết về nhà hàng. \\
\cmidrule(lr){2-3}
    & Thực đơn (Menu) \& Chi tiết món ăn
    & Hiển thị danh sách món ăn, combo và thông tin chi tiết. \\
\cmidrule(lr){2-3}
    & Đặt bàn (Table Booking)
    & Cho phép khách đặt bàn trước tại nhà hàng. \\
\cmidrule(lr){2-3}
    & Tự phục vụ (Self Order)
    & Cung cấp tính năng tự đặt món tại bàn qua QR code. \\
\cmidrule(lr){2-3}
    & Liên hệ (Contact Us)
    & Cung cấp thông tin liên hệ và biểu mẫu phản hồi. \\
\midrule

\textbf{Khách hàng (Đã đăng nhập)}
    & Quản lý đặt bàn (My Bookings)
    & Xem và quản lý các lịch đặt bàn của cá nhân. \\
\cmidrule(lr){2-3}
    & Đặt món trực tuyến (My Pre Orders)
    & Đặt món trước hoặc đặt giao hàng tận nơi. \\
\cmidrule(lr){2-3}
    & Quản lý đơn hàng (My Orders)
    & Theo dõi lịch sử và trạng thái các đơn hàng đã đặt. \\
\cmidrule(lr){2-3}
    & Gửi phản hồi (Feedback)
    & Đánh giá về nhà hàng, dịch vụ và món ăn. \\
\midrule

\textbf{Nhân viên}
    & \textbf{Tổng quan \& Cổng thông tin}
    & \\
    & Bảng điều khiển (App Dashboard)
    & Giao diện chính, truy cập các mô-đun theo vai trò. \\
    & Cổng thông tin nhân viên
    & Xem lịch làm, đăng ký ca, xin nghỉ phép. \\
\cmidrule(lr){2-3}
    & \textbf{Bếp (KDS - Kitchen Display System)}
    & \\
    & Bảng điều khiển KDS \& Giao diện Bếp
    & Hiển thị, theo dõi và cập nhật trạng thái các món ăn cần chuẩn bị. \\
\cmidrule(lr){2-3}
    & \textbf{Thu ngân (POS - Point of Sale)}
    & \\
    & Bảng điều khiển POS
    & Giao diện chính để xử lý đơn hàng và thanh toán. \\
    & Quản lý đơn hàng, Thanh toán, Quản lý bàn
    & Các chức năng cốt lõi cho việc vận hành tại quầy. \\
\cmidrule(lr){2-3}
    & \textbf{Quản lý (Manager)}
    & \\
    & Quản lý Thực đơn
    & Quản lý toàn bộ món ăn, danh mục, thuộc tính, và combo. \\
    & Quản lý Đặt chỗ
    & Xử lý và xác nhận các đơn hàng đặt trước và đặt bàn. \\
    & Quản lý Phản hồi
    & Xem, phân tích và phản hồi các đánh giá của khách hàng. \\
    & Quản lý Lịch làm việc
    & Tạo, phân công và quản lý lịch làm, ca làm cho nhân viên. \\
    & Cài đặt \& Hệ thống
    & Quản lý sơ đồ bàn, chi nhánh, nhân viên và phân quyền. \\
...
\begin{figure}[H]
	...
	\vspace{0.5cm}
	\caption{Lược đồ cơ sở dữ liệu vậy lý}
	\label{fig:logical_schema}
\end{figure}

\subsubsection{Truy vấn với Index}

Nhóm đã phân tích các luồng nghiệp vụ chính và xác định các cột thường xuyên được sử dụng trong các mệnh đề \texttt{WHERE}, \texttt{JOIN} và \texttt{ORDER BY} để tạo các chỉ mục phù hợp. Bảng dưới đây trình bày một số index tiêu biểu đã được triển khai và vai trò của chúng trong hệ thống.

\begin{table}[h!]
\centering
\caption{Mô tả một số Index tiêu biểu trong thiết kế vật lý}
\label{tab:index-description}
\begin{tabular}{|l|p{8cm}|}
\hline
\textbf{Tên Index} & \textbf{Mục đích \& Tác động} \\ \hline
\texttt{idx\_booking\_table\_customer\_phone} & Tăng tốc độ tìm kiếm thông tin đặt bàn của khách hàng qua số điện thoại. Đây là thao tác thường xuyên của nhân viên lễ tân. \\ \hline
\texttt{idx\_booking\_table\_time\_range} & Phục vụ truy vấn tìm kiếm bàn trống trong một khoảng thời gian cụ thể. Đây là index composite (ghép trên nhiều cột), tối quan trọng cho hiệu năng của chức năng đặt bàn online. \\ \hline
\texttt{idx\_pos\_orders\_branch\_status\_date} & Tối ưu hóa các truy vấn tổng hợp, báo cáo doanh thu theo chi nhánh, trạng thái và ngày tháng. Giúp các trang dashboard và báo cáo tải dữ liệu nhanh chóng. \\ \hline
\texttt{uc\_branch\_name} & Đảm bảo tính toàn vẹn dữ liệu, không cho phép tạo ra hai chi nhánh có tên trùng nhau. Đây là một \textit{unique index}. \\ \hline
\texttt{idx\_category\_parent} & Hỗ trợ truy vấn đệ quy hoặc các thao tác JOIN để hiển thị cây danh mục sản phẩm (cha-con) một cách hiệu quả, tránh việc quét toàn bộ bảng \texttt{category}. \\ \hline
\texttt{idx\_pos\_order\_items\_status} & Tăng tốc các truy vấn lọc món ăn trong đơn hàng theo trạng thái (ví dụ: đang chế biến, đã hoàn thành), thường dùng trong màn hình KDS của bếp và trang quản lý đơn hàng. \\ \hline
\end{tabular}
\end{table}

Bằng việc áp dụng chiến lược đánh index một cách có chủ đích, hệ thống Menu+ không chỉ đảm bảo tính đúng đắn về mặt logic mà còn được tối ưu hóa về mặt hiệu suất, sẵn sàng cho việc vận hành trong thực tế với khối lượng giao dịch lớn.